    ...
    <footer className="py-12 px-4 sm:px-6 lg:px-8 font-sans relative z-20 w-full">
      <div className="max-w-7xl mx-auto flex flex-col items-center relative z-10">
        <div className="mb-8 flex flex-col items-center justify-center text-center">
          <img src="/logo.png" alt="Prime Stone Builders" className="h-14 sm:h-20 w-auto mb-4 filter brightness-110 drop-shadow-2xl" />
          <p className="text-white/60 text-xs sm:text-sm font-medium tracking-wide uppercase mb-6">General Construction Services &bull; Exterior Solutions</p>
          
          <div className="flex flex-col md:flex-row items-center gap-3 sm:gap-4 md:gap-8 text-white/80 text-xs sm:text-sm md:text-base font-medium">
            ...
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 text-[#38bdf8]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
              <a href="mailto:contact@primestonebuilders.com" className="hover:text-[#38bdf8] transition-colors">contact@primestonebuilders.com</a>
            </div>
          </div>
        </div>

        <nav className="mb-8 w-full">
          <ul className="flex flex-wrap justify-center gap-x-6 sm:gap-x-8 gap-y-3 text-xs sm:text-sm md:text-base font-medium">
            {navLinks.map((link) => (
              <li key={link.name}>
                <a
                  href={link.href}
        ...
        <p className="text-center text-xs text-white/40 mt-4">
          &copy; {new Date().getFullYear()} Prime Stone Builders LLC. All rights reserved.
        </p>
      </div>
    </footer>
  );
